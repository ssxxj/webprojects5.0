...
from project_paths_v5 import project_relative, resolve_project_path  # noqa: E402
...
def process_lecture_governance(config_path: Path) -> dict[str, Any]:
        ...
        if manifest:
            ...
            if not isinstance(upstream, dict):
                ...
                for key in ("profile", "assignment_pack"):
                    ...
                    elif not resolve_project_path(str(target), PROJECT_ROOT).exists():
            ...
            if not isinstance(generated_assets, list) or not generated_assets:
                ...
                missing_generated = [
                    item for item in generated_assets
                    if not resolve_project_path(str(item), PROJECT_ROOT).exists()
                ]
    ...
    return {
        ...
        "issues": issues,
        "checked_files": {key: project_relative(path, PROJECT_ROOT) for key, path in expected.items() if key != "lecture_dir"},
    }
...
def run_lecture_governance(root: Path, chapter_filters: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "root": project_relative(LECTURE_ROOT, PROJECT_ROOT),
