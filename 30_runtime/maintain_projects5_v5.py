...
def run_build(root: Path, chapter_filters: list[str], validate_only: bool) -> dict[str, Any]:
    ...
    summary = build_all.build_summary(root, chapter_filters, validate_only, results)
    (root / "all_chapters_build_manifest_v5.json").write_text(
...
def run_drift(root: Path, chapter_filters: list[str], diff_lines: int) -> dict[str, Any]:
    ...
    summary = drift_check.build_summary(root, chapter_filters, diff_lines, results)
    (root / "all_chapters_drift_report_v5.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    return summary
